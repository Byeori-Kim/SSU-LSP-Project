...
void *threadRoutine(void *data)
{
    ...
    // work_data->*work_matrix = (int)calloc(4096, sizeof(int));
...
void thre_mode(int thr_num, int gen)
{
    ...
    struct work *work_data;
    // printf("    SIZE: %d\n", sizeof(struct work));
    work_data = calloc(1, sizeof(struct work));
    work_data->work_matrix = (int **)malloc(sizeof(int*)*2048);
    
    if(work_data->work_matrix == NULL)
    {
        printf("NOT ENOUGH MEMORY!\n");
    }else
    {
        // printf("MEMORY ASSIGNED: %d\n", (int)work_data->work_matrix);
    }

    for(int i=0; i<32; i++)
    {
        work_data->work_matrix[i] = (int *)malloc(sizeof(int)*32);
        if(work_data->work_matrix[i] == NULL)
        {
            printf("NOT ENOUGH MEMORY!\n");
        }else
        {
            // printf("  MEMORY ASSIGNED: %d\n", (int)work_data->work_matrix[i]);
        }
    }
